...
React 用の RatioKit スターターキットです。このディレクトリには、すぐに動作確認できるデモ環境と、既存プロジェクトへ導入するためのコンポーネントが含まれています。
...
2. **既存プロジェクトへ導入**: 必要なファイルを既存の React プロジェクトにコピーして使用
...
既存の React プロジェクトに以下のパッケージをインストールします：
