...
const openSetupModal = () => {
  ...
  const items: ContentItem[] = [
    {
      type: 'text',
      content: 'Vue版 RatioKit は、Vite 6+ および Tailwind CSS v4 環境に最適化されたスターターキット形式で提供されています。',
      title: 'Getting Started (Vite & Tailwind v4)'
    },
    {
      type: 'text',
      content: 'まず、上の「Download Zip」ボタンからファイルをダウンロードしてください。`Vue/` フォルダに必要なファイルがすべて含まれています。',
...
export default defineConfig({
  plugins: [
    ...
    {
      type: 'text',
      content: '`Vue/src/components/RatioKit` フォルダと `RatioKit.scss` を自身の `src/` 配下にコピーしてください。',
      title: 'Step 4: Copy Files'
    },
    {
      type: 'text',
      content: '`main.ts` 等で SCSS をインポートします。Tailwind v4 の `@import "tailwindcss";` より後に読み込むことで、RatioKit のベーススタイルが正しく適用されます。',
...
import './RatioKit.scss';`
  ...
  --gap: 30px;   /* 余白 */
    ...
    {
      type: 'text',
      content: 'あとはカタログからコードをコピーして使うだけです。Vue 3 の SFC（Single File Component）として利用できます。',
      title: 'Usage Example'
    },
    {
      type: 'code',
...
<template>
  ...
  copyStates.value = initialCopyStates;
  showModal.value = true;
  document.body.style.overflow = 'hidden';
};
...
<template>
  <div v-if="showModal" id="snippetModal" class="active" @click="closeModal">
    <div class="code-editor" :class="{ 'setup-mode': isSetup }" @click.stop>
      <div class="code-header text-sm flex items-center gap-2">
        ...
        <button v-if="!isSetup && modalItems.length > 0" class="copy-btn" @click="handleCopy(0, modalItems[0].content)">
          {{ copyStates[0] || 'Copy' }}
        </button>
