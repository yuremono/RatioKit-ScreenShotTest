...
- [Package Distribution Skill](./.skills/package-distribution/SKILL.md): 配布パッケージ（Zip）の更新・デプロイ手順。過去の失敗から学んだ教訓を含む。
...
- **スキルファイル参照の徹底**: タスク実行前に該当する`.skills/`ディレクトリのスキルファイルを確認し、過去の失敗パターンや推奨プロセスを把握する。特に「zipファイル更新」「配布パッケージ更新」の依頼時は、必ず [Package Distribution Skill](./.skills/package-distribution/SKILL.md) を参照すること。
